...
for (let i = 0; i < lines.length; i++) {
  ...
  // We look for JSX tags like <div>, <button>, etc. and closing tags like </div>, </button>
  // Also self-closing tags like <img ... /> should not affect the stack.
  ...
  for (const match of matches) {
    ...
    if (full.endsWith('/>') || tag.startsWith('polygon') || tag.startsWith('line') || tag.startsWith('path') || tag.startsWith('img') || tag.startsWith('input') || tag.startsWith('hr') || tag.startsWith('br') || tag.startsWith('rect') || tag.startsWith('svg') || tag.startsWith('circle') || tag.startsWith('link')) {
      continue;
    }
