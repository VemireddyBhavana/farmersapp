...
def replace_tamil_leakage(text, lang, target_text):
    ...
    # Replace the Tamil block within this language block
    new_content, count = tamil_block_regex.subn(target_text, block_content)
...
# Also fix the smartFarmingTools and missing duplicate removing for agriKnowledgeHub
agri_knowledge_hub_title_pattern = re.compile(r'\s*agriKnowledgeHubTitle:\s*".*?",\n')
def remove_duplicate_title(text, lang):
    block_pattern = re.compile(rf'  {lang}: {{(.*?)\n  }},?\n', re.DOTALL)
    match = block_pattern.search(text)
    if not match:
        return text
    ...
    matches = list(agri_knowledge_hub_title_pattern.finditer(block_content))
    if len(matches) > 1:
        for m in reversed(matches[1:]):
            block_content = block_content[:m.start()] + block_content[m.end():]
        print(f"Removed duplicate agriKnowledgeHubTitle from {lang}")
    return text[:match.start(1)] + block_content + text[match.end(1):]

for lang in ["English", "Telugu", "Hindi", "Tamil", "Marathi", "Gujarati", "Kannada", "Malayalam", "Punjabi", "Bangla"]:
    content = remove_duplicate_title(content, lang)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Done fixing leakage and removing duplicates.")
