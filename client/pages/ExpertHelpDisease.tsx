...
export default function ExpertHelpDisease() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setError("Image file is too large. Max size is 4MB.");
      return;
    }
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Failed to run disease detection API. Please try again.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-8">
        ...
        <div className="space-y-2">
          <h2 className="text-3xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white leading-tight">
            {t("cropDiseaseDetection") || "Crop Disease Detection"}
          </h2>
          <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">
            Upload a high-clear photo of your crop's infected leaves. Our AI system will diagnose symptoms and generate organic or chemical remedies immediately.
          </p>
        </div>
                ...
                >
                  {preview ? (
                    <div className="w-full h-full space-y-4">
                      ...
                      />
                      <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Tap to replace image</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1">
                        <p className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Tap to upload crop photo</p>
                        <p className="text-xs font-semibold text-slate-400">Accepts JPEG or PNG (Max 4MB)</p>
                      </div>
                    </div>
                  )}
                </label>
          ...
          <div className="md:col-span-7">
            {loading ? (
              <div className="h-full min-h-[350px] bg-white dark:bg-slate-900 rounded-[2.5rem] p-10 border dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center space-y-6">
                ...
                <div className="space-y-1">
                  <p className="text-base font-black uppercase tracking-widest text-slate-800 dark:text-white">Analyzing your crop photo...</p>
                  <p className="text-xs font-semibold text-emerald-600 animate-pulse">Running Agricultural Vision model</p>
                </div>
              </div>
            ) : diagnosis ? (
              <motion.div
              ...
              >
                <div className="flex items-center gap-3 border-b dark:border-slate-800 pb-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase italic tracking-tight">AI Diagnosis Results</h3>
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Analysis complete</p>
                  </div>
                </div>
              ...
              <div className="h-full min-h-[350px] border-4 border-dashed border-slate-100 dark:border-slate-800/40 rounded-[2.5rem] flex flex-col items-center justify-center text-slate-300 dark:text-slate-700 p-8 text-center bg-white/50 dark:bg-slate-900/10">
                ...
                <p className="text-xs font-black uppercase tracking-widest leading-relaxed max-w-xs opacity-60">
                  Please upload a photo and click Detect Disease to run diagnosis
                </p>
              </div>
            )}
          </div>
