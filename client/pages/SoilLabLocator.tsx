    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-20 pb-12">
      <div className="container mx-auto px-4">
        ...
        <motion.div 
            ...
            className="max-w-7xl mx-auto mb-16 pt-10 px-6"
        >
            <div className="bg-[#064e3b] rounded-[4rem] p-16 lg:p-24 relative overflow-hidden shadow-3xl">
                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/topography.png')] opacity-20" />
                <div className="absolute top-0 right-0 p-16 opacity-10 antigravity-float">
                    <Microscope className="h-[25rem] w-[25rem] text-emerald-400" />
                </div>
                
                <div className="relative z-10 max-w-3xl space-y-8">
                    <div className="flex items-center gap-4">
                        <Badge className="bg-emerald-500 text-white border-none px-6 py-2 rounded-2xl font-black uppercase text-[11px] tracking-[0.4em] italic shadow-xl shadow-emerald-500/20">
                            ISMIGS National Lab Network
                        </Badge>
                        <div className="h-px w-24 bg-emerald-500/30 hidden md:block" />
                    </div>
                    
                    <h1 className="text-5xl lg:text-8xl font-black tracking-tighter text-white uppercase italic leading-[0.85]">
                        Soil Intelligence <span className="text-emerald-400">Command Center</span>
                    </h1>
                    
                    <p className="text-xl text-emerald-100/70 font-bold uppercase tracking-widest leading-relaxed max-w-2xl">
                        Locating certified laboratories and deploying <span className="text-white">Neural-NPK</span> diagnostics for high-yield soil optimization.
                    </p>

                    <div className="flex flex-wrap gap-6 pt-4">
                        <div className="flex -space-x-4">
                            {[1,2,3,4].map(i => (
                                <div key={i} className="h-12 w-12 rounded-full border-4 border-[#064e3b] bg-emerald-800 flex items-center justify-center overflow-hidden">
                                    <img src={`https://i.pravatar.cc/150?u=soil${i}`} alt="user" />
                                </div>
                            ))}
                        </div>
                        <div className="flex flex-col justify-center">
                            <p className="text-white font-black italic text-sm">4,800+ Labs Registered</p>
                            <p className="text-emerald-400/60 text-[10px] font-bold uppercase tracking-widest">Verified by Ministry of Agriculture</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8 max-w-7xl mx-auto">
          <div className="lg:col-span-4 space-y-8">
            <div className="relative group">
              ...
              <Search className={cn("absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 transition-colors z-10", searchQuery ? "text-emerald-500" : "text-slate-400")} />
              <input 
                type="text" 
                placeholder="Enter Pincode for Regional Data..." 
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full h-20 pl-16 pr-6 bg-white dark:bg-slate-900 border-2 border-emerald-50 dark:border-slate-800 shadow-2xl rounded-[2rem] text-sm font-black italic outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all font-mono z-10 relative"
              />
              {searchQuery && (
                <button 
                  onClick={() => setSearchQuery("")}
                  className="absolute right-8 top-1/2 -translate-y-1/2 text-[10px] font-black uppercase tracking-widest text-rose-500 hover:text-rose-600 z-20"
                >
                  Clear Vector
                </button>
              )}
            ...
            <div className="space-y-6 pt-4">
              <div className="flex items-center justify-between px-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 italic">Regional Facilities</h4>
                <Badge variant="outline" className="border-emerald-500/20 text-emerald-600 font-black text-[9px] px-3">{filteredLabs.length} Active</Badge>
              </div>
              
              {filteredLabs.length > 0 ? filteredLabs.map((lab) => (
                <Card key={lab.id} className="p-8 rounded-[3rem] border-2 border-emerald-50/50 dark:border-slate-800 shadow-2xl bg-white dark:bg-slate-900 group hover:border-emerald-500 transition-all cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className="relative z-10">
                      <div className="flex items-center gap-3 mb-2">
                        <h4 className="text-xl font-black text-slate-900 dark:text-white italic uppercase tracking-tighter group-hover:text-emerald-600 transition-colors">{lab.name}</h4>
                        {lab.certified && <CheckCircle2 className="h-5 w-5 text-emerald-500" />}
                      </div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic flex items-center gap-2">
                        <MapPin className="h-3 w-3" /> {lab.pincode} • <Calendar className="h-3 w-3" /> {lab.time}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                     {lab.tags.map((tag, i) => (
                       <span key={i} className="px-4 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-[9px] font-black uppercase tracking-widest text-emerald-600 italic border border-emerald-100/50">
                         {tag}
                       </span>
                     ))}
                  </div>
                  ...
                  <div className="flex gap-3">
                    <Button 
                      ...
                      className="flex-1 h-14 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 hover:bg-slate-50"
                    >
                      <Phone className="mr-2 h-4 w-4" /> Call Unit
                    </Button>
                    <Button 
                      onClick={() => setSelectedLabForBooking(lab)}
                      className="flex-1 h-14 bg-[#064e3b] hover:bg-emerald-800 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-900/20"
                    >
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-16 text-center bg-white dark:bg-slate-900 rounded-[3rem] border-4 border-dashed border-slate-100 dark:border-slate-800">
                   <div className="h-20 w-20 bg-rose-50 dark:bg-rose-900/10 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-rose-500/20">
                      <AlertTriangle className="h-10 w-10" />
                   </div>
                   <h5 className="text-xl font-black italic text-slate-900 dark:text-white uppercase tracking-tighter">No Centers in {searchQuery}</h5>
                   <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">Check Regional Pincode Buffer</p>
                </motion.div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            <Card className="p-0 rounded-[4rem] border-4 border-white dark:border-slate-800 shadow-3xl bg-slate-100 overflow-hidden h-[500px] relative">
                   ...
                   <Marker key={lab.id} position={lab.coords}>
                     <Popup>
                       <div className="p-4 min-w-[200px]">
                          <Badge className="bg-emerald-600 text-white mb-2 rounded-lg">Verified Unit</Badge>
                          <h4 className="text-lg font-black italic text-slate-900 uppercase tracking-tight leading-none mb-1">{lab.name}</h4>
                          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-1">
                            <Navigation className="h-3 w-3" /> {lab.time}
                          </p>
                          <Button 
                            className="h-10 w-full bg-[#064e3b] text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg"
               ...
               <div className="absolute bottom-8 right-8 z-[1000] p-6 bg-white/90 backdrop-blur-xl rounded-[2.5rem] shadow-3xl border-2 border-emerald-500/20 max-w-[240px]">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="h-2 w-2 bg-emerald-500 rounded-full animate-ping" />
                    <h3 className="text-xs font-black uppercase italic tracking-tighter text-slate-900">Live Coverage Map</h3>
                  </div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-700 mb-2">{filteredLabs.length} Facilities in Buffer</p>
                  <p className="text-[9px] text-slate-500 font-medium leading-relaxed">Integrated with ISMIGS Geospatial lab database for real-time verification.</p>
               </div>
            </Card>

            <Card className="p-12 rounded-[4rem] border-none shadow-3xl bg-white dark:bg-slate-900">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12">
                   <div className="flex items-center gap-5">
                      <div className="h-16 w-16 bg-[#064e3b] text-emerald-400 rounded-3xl flex items-center justify-center shadow-2xl shadow-emerald-900/20">
                        {analysisMode === 'npk' ? <Microscope className="h-8 w-8" /> : <Layers3 className="h-8 w-8" />}
                      </div>
                      <div>
                        <h3 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-none mb-1">
                          {analysisMode === 'npk' ? 'Neural NPK Analyzer' : 'Structural Dynamics'}
                        </h3>
                        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-600/60 italic">ISMIGS AI Diagnostic Engine</p>
                      </div>
                   </div>

                   <div className="flex bg-slate-100 dark:bg-slate-800 p-2 rounded-[1.5rem] shadow-inner">
                      <button 
                        onClick={() => setAnalysisMode('npk')}
                        className={cn("px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all", analysisMode === 'npk' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-xl" : "text-slate-400 hover:text-slate-600")}
                      >
                        NPK Vector
                      </button>
                      <button 
                        onClick={() => setAnalysisMode('structure')}
                        className={cn("px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all", analysisMode === 'structure' ? "bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-xl" : "text-slate-400 hover:text-slate-600")}
                      >
                        Architecture
                      </button>
                   </div>
                 ...
                 {analysisMode === 'structure' ? (
                    ...
                    >
                      ...
                      <div className="grid md:grid-cols-3 gap-6">
                         {[
                           { icon: Wind, title: "Aeration Dynamics", value: structuralScore > 70 ? "High Porosity" : "Balanced", color: "text-blue-600 bg-blue-50 dark:bg-blue-900/20 border-blue-100" },
                           { icon: Cloud, title: "Porosity Vector", value: (organicMatter * 8.2).toFixed(1) + "%", color: "text-purple-600 bg-purple-50 dark:bg-purple-900/20 border-purple-100" },
                           { icon: Sprout, title: "Root Architecture", value: structuralScore + "/100 Index", color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100" }
                         ].map((card, i) => (
                           <Card key={i} className="p-8 rounded-[3rem] border-2 border-transparent hover:border-emerald-500/30 shadow-2xl bg-white dark:bg-slate-800 flex items-center gap-5 group hover:scale-[1.05] transition-all duration-500">
                              <div className={cn("h-16 w-16 rounded-[1.5rem] flex items-center justify-center shrink-0 shadow-lg", card.color)}>
                                 <card.icon className="h-8 w-8" />
                              </div>
                              <div>
                                 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 italic mb-1">{card.title}</p>
                                 <h5 className="text-xl font-black italic text-slate-900 dark:text-white tracking-tighter leading-none">{card.value}</h5>
                              </div>
                           </Card>
                         ))}
                      </div>
                   ...
                   <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                         {[
                           { label: "Nitrogen (N)", value: analysisResult.nitrogen, color: "text-rose-600 bg-rose-50 dark:bg-rose-500/10 border-rose-100" },
                           { label: "Phosphorus (P)", value: analysisResult.phosphorus, color: "text-amber-600 bg-amber-50 dark:bg-amber-500/10 border-amber-100" },
                           { label: "Potassium (K)", value: analysisResult.potassium, color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100" },
                           { label: "Soil pH", value: analysisResult.ph, color: "text-blue-600 bg-blue-50 dark:bg-blue-500/10 border-blue-100" }
                         ].map((item, i) => (
                           <div key={i} className={cn("p-6 rounded-[2rem] border-2 shadow-xl", item.color)}>
                             <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-3 italic">{item.label}</p>
                      ...
                      <div className="grid lg:grid-cols-2 gap-8">
                        ...
                        <div className={cn("p-8 rounded-[3rem] border-2 transition-all shadow-2xl", fertilizerTiming.safe ? "bg-emerald-50 dark:bg-emerald-900/10 border-emerald-500/30" : "bg-orange-50 dark:bg-orange-900/10 border-orange-500/30")}>
                          <div className="flex items-center gap-4 mb-4">
                             <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center shadow-lg", fertilizerTiming.safe ? "bg-emerald-500 text-white" : "bg-orange-500 text-white")}>
                               {fertilizerTiming.safe ? <CheckCircle2 className="h-6 w-6" /> : <AlertTriangle className="h-6 w-6" />}
                             </div>
                             <div>
                               <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 italic leading-none mb-1">Atmospheric Sync</p>
                               <h4 className="text-xl font-black italic text-slate-900 dark:text-white uppercase tracking-tight">Window: {fertilizerTiming.safe ? "Optimal" : "Warning"}</h4>
                             </div>
                          </div>
                        ...
                        {/* Recommendation Card */}
                        <div className="p-8 rounded-[3rem] bg-slate-900 dark:bg-slate-800 border-2 border-slate-800 shadow-2xl relative overflow-hidden">
                          <div className="absolute top-0 right-0 p-8 opacity-10">
                            <Zap className="h-16 w-16 text-emerald-400" />
                          </div>
                          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-400 mb-4 italic">Neural Advisory Profile</p>
                          <p className="text-sm font-bold text-white dark:text-emerald-50 leading-relaxed italic relative z-10">{analysisResult.recommendation}</p>
                        </div>
                      </div>
